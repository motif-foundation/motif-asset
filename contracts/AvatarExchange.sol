...
contract AvatarExchange is IAvatarExchange {
    ...
    modifier onlyAvatarCaller() {
        require(avatarContract == msg.sender, "AvatarExchange: Only avatar contract allowed");
        _;
    }
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "AvatarExchange: Invalid bid shares for token"
        );
        return
            bidAmount != 0 &&
            (bidAmount ==
        ...
        onlyAvatarCaller
    {
        require(
            ...
            "AvatarExchange: Ask invalid for share splitting"
        );
    ...
    function removeAsk(uint256 tokenId) external override onlyAvatarCaller {
        emit AskRemoved(tokenId, _tokenAsks[tokenId]);
        delete _tokenAsks[tokenId];
    }
 
    function configure(address avatarContractAddress) external override {
        require(msg.sender == _owner, "AvatarExchange: only owner allowed");
        require(avatarContract == address(0), "AvatarExchange: already configured");
        require(
            avatarContractAddress != address(0),
            "AvatarExchange: cannot set avatar contract as zero address"
        );

        avatarContract = avatarContractAddress;
    }
        ...
        onlyAvatarCaller
    {
        require(
            isValidBidShares(bidShares),
            "AvatarExchange: Invalid bid shares, must sum to 100"
        );
    ...
    function setBid(
        ...
        Bid memory bid,
        address spender
    ) public override onlyAvatarCaller {
        BidShares memory bidShares = _bidShares[tokenId];
        require(
            bidShares.creator.value.add(bid.sellOnShare.value) <=
                uint256(100).mul(Decimal.BASE),
            "AvatarExchange: Sell on fee invalid for share splitting"
        );
        require(bid.bidder != address(0), "AvatarExchange: bidder cannot be 0 address");
        require(bid.amount != 0, "AvatarExchange: cannot bid amount of 0");
        require(
            bid.currency != address(0),
            "AvatarExchange: bid currency cannot be 0 address"
        );
        require(
            bid.recipient != address(0),
            "AvatarExchange: bid recipient cannot be 0 address"
        );
        ...
        onlyAvatarCaller
    {
        ...
        require(bid.amount > 0, "AvatarExchange: cannot remove bid amount of 0");
        ...
        onlyAvatarCaller
    {
        ...
        require(bid.amount > 0, "AvatarExchange: cannot accept bid of 0");
        require(
            ...
            "AvatarExchange: Unexpected bid found."
        );
        require(
            isValidBid(tokenId, bid.amount),
            "AvatarExchange: Bid invalid for share splitting"
        );
    ...
    function _finalizeNFTTransfer(uint256 tokenId, address bidder) private {
        ...
        token.safeTransfer(
            IERC721(avatarContract).ownerOf(tokenId),
            splitShare(bidShares.owner, bid.amount)
        );
 
        token.safeTransfer(
            Avatar(avatarContract).tokenCreators(tokenId),
            splitShare(bidShares.creator, bid.amount)
        );

        token.safeTransfer(
            Avatar(avatarContract).previousTokenOwners(tokenId),
            splitShare(bidShares.prevOwner, bid.amount)
        );
        Avatar(avatarContract).listTransfer(tokenId, bid.recipient);
