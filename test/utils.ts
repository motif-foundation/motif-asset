import { BaseErc20Factory, ItemFactory } from '../typechain';
...
export async function signPermit(
  ...
  return new Promise<EIP712Sig>(async (res, reject) => {
    let nonce;
    const itemContract = ItemFactory.connect(tokenAddress, owner);

    try {
      nonce = (
        await itemContract.permitNonces(owner.address, tokenId)
      ).toNumber();
    } catch (e) {
      ...
      const sig = signTypedData(Buffer.from(owner.privateKey.slice(2), 'hex'), {
        data: {
          ...
          domain: {
            ...
            version: '1',
            chainId,
            verifyingContract: itemContract.address,
          },
          message: {
            spender: toAddress,
            tokenId,
...
export async function signMintWithSig(
  ...
  return new Promise<EIP712Sig>(async (res, reject) => {
    ...
    try {
      nonce = (await itemContract.mintWithSigNonces(creator)).toNumber();
    } catch (e) {
      console.error('NONCE', e);
      reject(e);
      return;
