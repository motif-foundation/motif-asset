...
import { Item } from '../typechain/Item';
import {
  approveCurrency,
  deployCurrency,
  EIP712Sig,
...
describe('Item', () => {
  ...
  async function tokenAs(wallet: Wallet) {
    return ItemFactory.connect(tokenAddress, wallet);
  }
  async function deploy() {
    ...
    const token = await (
      await new ItemFactory(deployerWallet).deploy(list.address, "Motif",  "MOTIF")
    ).deployed();
    tokenAddress = token.address;

    await list.configure(tokenAddress);
  }
...
// WRITE LATER
/*  async function mintForCreatorWithoutSig(
...
/*  async function mintWithSig(
  ...
  async function setAsk(token: Item, tokenId: number, ask: Ask) {
    return token.setAsk(tokenId, ask);
  }

  async function removeAsk(token: Item, tokenId: number) {
    return token.removeAsk(tokenId);
  }

  async function setBid(token: Item, bid: Bid, tokenId: number) {
    return token.setBid(tokenId, bid);
  }

  async function removeBid(token: Item, tokenId: number) {
    return token.removeBid(tokenId);
  }

  async function acceptBid(token: Item, tokenId: number, bid: Bid) {
    return token.acceptBid(tokenId, bid);
  }

  // Trade a token a few times and create some open bids
  async function setupList(currencyAddr: string, tokenId = 0) {
    ...
    await approveCurrency(currencyAddr, listAddress, creatorWallet);
    await approveCurrency(currencyAddr, listAddress, prevOwnerWallet);
    await approveCurrency(currencyAddr, listAddress, ownerWallet);
    await approveCurrency(currencyAddr, listAddress, bidderWallet);
    await approveCurrency(currencyAddr, listAddress, otherWallet);
  ...
  describe('#mint', () => {
    ...
    it('should revert if an empty content hash is specified', async () => {
      ...
      ).rejectedWith('Item: content hash must be non-zero');
    });
    ...
    it('should revert if the metadataHash is empty', async () => {
      ...
      ).rejectedWith('Item: metadata hash must be non-zero');
    });
    ...
    it('should revert if the tokenURI is empty', async () => {
      ...
      ).rejectedWith('Item: specified uri must be non-empty');
    });
 ...
 /* describe('#mintWithSig', () => {
    ...
    it('should mint a token for a given creator with a valid signature', async () => {
      ...
      const itemExchange = await ItemExchangeFactory.connect(listAddress, otherWallet);
      ...
      const recoveredCreatorBidShare = formatUnits(
        (await itemExchange.bidSharesForToken(0)).creator.value,
        'ether'
      );
    ...
    it('should not mint a token for a different creator', async () => {
      ...
      ).rejectedWith('Item: Signature invalid');
    });
    ...
    it('should not mint a token with an invalid deadline', async () => {
      ...
      ).rejectedWith('Item: mintWithSig expired');
    });
  });*/
  ...
  describe('#removeAsk', () => {
    it('should remove the ask', async () => {
      const token = await tokenAs(ownerWallet);
      const itemExchange = await ItemExchangeFactory.connect(
        listAddress,
        deployerWallet
      );
      ...
      const ask = await itemExchange.currentAskForToken(0);
    ...
    it('should emit an Ask Removed event', async () => {
      const token = await tokenAs(ownerWallet);
      const list = await ItemExchangeFactory.connect(
        listAddress,
        deployerWallet
      );
      await setAsk(token, 0, defaultAsk);
      ...
      const events = await list.queryFilter(
        list.filters.AskRemoved(0, null),
        block
      );
  ...
  describe('#setBid', () => {
    ...
    it('should automatically transfer the token if the ask is set', async () => {
      const token = await tokenAs(bidderWallet);
      const asOwner = await tokenAs(ownerWallet);
      await setupList(currencyAddr, 1);
  ...
  describe('#removeBid', () => {
    ...
    it('should revert if the bidder has not placed a bid', async () => {
      ...
      await expect(removeBid(token, 0)).rejectedWith(
        'ItemExchange: cannot remove bid amount of 0'
      );
    });
  ...
  describe('#acceptBid', () => {
    ...
    it('should accept a bid', async () => {
      const token = await tokenAs(ownerWallet);
      const list = await ItemExchangeFactory.connect(listAddress, bidderWallet);
      const asBidder = await tokenAs(bidderWallet);
      const bid = {
      ...
      const bidShares = await list.bidSharesForToken(0);
    ...
    it('should emit a bid finalized event if the bid is accepted', async () => {
      ...
      const events = await list.queryFilter(
        list.filters.BidFinalized(null, null),
        block
      );
      expect(events.length).eq(1);
      const logDescription = list.interface.parseLog(events[0]);
    ...
    it('should revert if not called by the owner', async () => {
      ...
      ).rejectedWith('Item: Only approved or owner');
    });
    ...
    it('should revert if a non-existent bid is accepted', async () => {
      ...
      ).rejectedWith('ItemExchange: cannot accept bid of 0');
    });
  ...
  describe('#transfer', () => {
    ...
    it('should remove the ask after a transfer', async () => {
      const token = await tokenAs(ownerWallet);
      const list = ItemExchangeFactory.connect(listAddress, deployerWallet);
      await setAsk(token, 0, defaultAsk);

      await expect(
        token.transferFrom(ownerWallet.address, otherWallet.address, 0)
      ).fulfilled;
      const ask = await list.currentAskForToken(0);
  ...
  describe('#burn', () => {
    ...
    it('should revert when the caller is not the owner or a creator', async () => {
      ...
      await expect(token.burn(0)).rejectedWith('Item: Only approved or owner');
    });

    it('should revert if the token id does not exist', async () => {
      const token = await tokenAs(creatorWallet);

      await expect(token.burn(100)).rejectedWith('Item: nonexistent token');
    });
 ...
 /* describe('#permit', () => {
  ...
  describe('#supportsInterface', async () => {
    ...
    it('should return true to supporting new metadata interface', async () => {
      const token = await tokenAs(otherWallet);
      const interfaceId = ethers.utils.arrayify('0xc904b9c6');
